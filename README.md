...
- 不同版本的算法不兼容。编辑器导入待解码图片时会尝试判断 v8c、v8、v7，并选中对应版本；识别不确定或使用更早版本时，请手动选择对应版本。
...
在解码界面拖入、粘贴或选择图片后，编辑器会自动判断并选中具体算法按钮，不提供独立的“自动识别”档位。随后按选中版本解码，手动切换和重新扫描区域不会覆盖你的版本选择。无法判断或图中混用多个版本时提示手动选择。扩展仍提供“自动识别”选项；已有的手动算法设置会保留。
自动识别比较试解后重新编码的基础层误差，不以“解码没有报错”作为成功依据。密码错误、区域偏移或候选结果接近时会提示无法可靠解码，不会自动切换密码。
...
```ts
import { decodeImageQcAuto } from "q_____c"

// 保留输入 alpha，不要预先按猜测的版本修改标记。
const { version, image, candidates, validationError } = await decodeImageQcAuto(carrier, password)
```

自动模式适用于当前 v8c、v8、v7 的未缩放 PNG/JPEG 载体，区域至少为 32×32。
它先在有界探针上试解，再验证完整区域；无法确定时抛出 `ImageQcAutoDecodeError`，其中包含候选误差。
旧格式没有版本校验和，自动判断属于启发式判据，不是密码认证，也不承诺识别任意缩放或严重损坏的载体。更早版本和不确定结果仍可手动解码。
